...
public class Q3_B_ClosestPointsToOrigin {

    public static int[][] solve(int[][] A, int B) {
        Restaurant[] restaurants = new Restaurant[A.length];
        for (int i = 0; i < A.length; i++) {
            restaurants[i] = new Restaurant(A[i][0], A[i][1]);
        }

        Arrays.sort(restaurants, Comparator.comparingInt(r -> r.distance));

        int[][] result = new int[B][2];
        for (int i = 0; i < B; i++) {
            result[i][0] = restaurants[i].x;
            result[i][1] = restaurants[i].y;
        }
        return result;
