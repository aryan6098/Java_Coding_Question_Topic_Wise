...
public class Q1_ParkingIceCreamTruck {
    public static ArrayList<Integer> slidingMaximum(final List<Integer> A, int B) {
        Deque<Integer> deque = new ArrayDeque<>();
        ArrayList<Integer> arrayList = new ArrayList<>();
        for(int i = 0; i < B; i++) {
            while (deque.size() > 0 && A.get(i) > deque.getLast()) {
                deque.removeLast();
            }
        ...
        int e = B;
        while(e < A.size()) {
            if(A.get(s-1) == deque.peekFirst()) {
                deque.removeFirst();
            }
            while (deque.size() > 0 && A.get(e) > deque.getLast()) {
                deque.removeLast();
            }
    ...
    public static void main(String[] args) {
        ...
        // ArrayList<Integer> arrayList = new ArrayList<>(Arrays.asList(1, 3, -1, -3, 5, 3, 6));

        // Convert int[] to List<Integer>
        List<Integer> listA = Arrays.stream(A).boxed().collect(Collectors.toList());

        // Call the slidingMaximum method with listA and B
        // ArrayList<Integer> arrayList = slidingMaximum(listA, 3);
        int B = 3;
        ArrayList<Integer> arrayList = slidingMaximum(listA, B);
        System.out.println(arrayList);

        int[] maxSlide = {1,3,-1,-3,5,3,6,7};
        int[] res = maxSlidingWindow(maxSlide, B);
        for(int i = 0; i < maxSlide.length; i++) {
